...
$stmt = $pdo->prepare("SELECT * FROM survey_results  ORDER BY submission_date DESC LIMIT 12");
$stmt->execute();
...
<td><span class="pen"><i class='bx bx-edit-alt'></i></span></td>
    ...
    <td style="background-color: blanchedalmond;">
        <span class="calender" id="identity_score" data-score="<?php echo $result["identity_score"]; ?>">
            <?php //echo $result["identity_score"]; ?> 
            <!-- <i class='bx bxs-calendar'></i> -->
        </span>
    </td>
    <td style="background-color: blanchedalmond;">
        <span class="calender" id="device_score" data-score="<?php echo $result["device_score"]; ?>">
            <?php //echo $result["device_score"]; ?>
        </span>
    </td>
    <td style="background-color: blanchedalmond;">
        <span class="calender" id="infra_score" data-score="<?php echo $result["infra_score"]; ?>">
            <?php //echo $result["infra_score"]; ?> 
        </span>
    </td>
    <td style="background-color: blanchedalmond;">
        <span class="calender" id="data_score" data-score="<?php echo $result["data_score"]; ?>">
            <?php // echo $result["data_score"]; ?>
        </span>
    </td>
    <td style="background-color: blanchedalmond;">
        <span class="calender" id="app_score" data-score="<?php echo $result["app_score"]; ?>">
            <?php //echo $result["app_score"]; ?>
        </span>
    </td>
    <td style="background-color: blanchedalmond;">
        <span class="calender" id="network_score" data-score="<?php echo $result["network_score"]; ?>">
...
<!-- maturity -->
    ...
    let scoreElements = document.querySelectorAll(".calender");

    scoreElements.forEach(element => {
        let score = parseInt(element.getAttribute("data-score").trim());
        let maturityLevel = "";
        let iconClass = "";
        let color = "";
        ...
        if (advancedOptimalCount <= 4) {
            ...
            color = "red"; 
            iconClass = "bx bx-x-circle";  // Red cross icon
        } else if (advancedOptimalCount <= 8) {
            maturityLevel = "Initial (Basic Security)";
            color = "yellow";
            iconClass = "bx bx-block";  // Yellow block icon
        } else if (advancedOptimalCount <= 11) {
            maturityLevel = "Advanced (Mature Zero Trust)";
            color = "green";
            iconClass = "bx bxs-check-circle"; // Green check icon
        } else {
            maturityLevel = "Optimal (AI-Driven)";
            color = "blue";
            iconClass = "bx bxs-check-circle"; // Blue check icon
        }

        // Update each score display dynamically
        element.innerHTML += ` <i class="${iconClass}" style="color:${color};"></i> <span style="color:${color}; font-weight:bold;"></span>`;
    });
});
